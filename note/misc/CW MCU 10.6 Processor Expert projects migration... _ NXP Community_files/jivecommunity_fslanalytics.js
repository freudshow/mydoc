...
jQuery(document).ready(function() {
	...
	if(jQuery('#j-search-input').length>0){
		searchTerm = jQuery('#j-search-input').val(); //No updates Required
	}

	if(jQuery('header#j-compact-header img.jive-avatar').length > 0){ //PreCloud Migration if(jQuery('header#j-header img.jive-avatar').length > 0){ //No updates Required
		userid = 0;
		username = jQuery('header#j-compact-header img.jive-avatar').attr('data-username'); //PreCloud Migration username = jQuery('header#j-header img.jive-avatar').attr('data-username'); //No updates Required
	}else{
		userid = 0;
		username = "Guest";
	}
	...
	if((jQuery('nav#j-placeNav').length != 0 || jQuery('nav ul.j-tabbar').length != 0) && jQuery('#j-globalNav .active').length <= 0){   // PreCloud Migration - if(jQuery('nav.j-bigtab-nav').length != 0 && jQuery('#j-globalNav .active').length <= 0){
		...
		if((jQuery('#jive-body-main').length > 0 || jQuery('#jive-body').length>0) && jQuery('#j-globalNav .active').length <= 0 && jQuery('.jive-create-large').length <=0){
			...
			contentType = jQuery('#jive-body-intro .j-context a').first().text(); //No Updates Required
			...
			category = jQuery("span.jive-content-footer-item:not(.jive-content-footer-tags,.jive-vid-tags) span:not('.jive-video-views-count')").text(); //No Updates Required
	...
	/* Pre Cloud Migration
		jQuery('.js-link-delete').click(function(){
			...
			// PreCloud Migration - Webtrends Removal jQuery('meta[id=community_activity_type]').attr('content', activityType);
			jQuery.getScript( "/themes/freescale_theme_v2_0/scripts/FSL_COMM_SMARTSOURCE_TAG.js" );
		});
	 */
	...
	jQuery('div.j-contained-tabs-place.j-browse-content div.j-sidebar-left nav.j-second-nav-categories li').click(function(){
		...
		// PreCloud Migration - Webtrends Removal jQuery('meta[id=community_category]').attr('content', category);
		// PreCloud Migration - Webtrends Removal jQuery.getScript( "/themes/freescale_theme_v2_0/scripts/FSL_COMM_SMARTSOURCE_TAG.js" );
	});
	...
	jQuery.getScript( "//assets.adobedtm.com/f2bd0c145f764c0e7b6dae5f9269061e38fabacd/satelliteLib-2fb7098ae4e0938976f1eee822973607d3f48c60.js", function(){
				...
				if(pageURL.indexOf(strKey) > -1){
					if(pageURL.indexOf("/tags#/") > -1){
						/* Pre Cloud
								...
								var pageHead = jQuery("header.j-page-header > h1 > a").first().text();
								currentSpace = jQuery.trim(pageHead.replace("Browse all tags in",""));

								if(jQuery("header.j-page-header > h1 > span.j-page-header-parent > a").length > 0){
									parentSpace = jQuery("header.j-page-header > h1 > span.j-page-header-parent > a").first().text();
		...
		if(isErrorPage){
			...
			if(jQuery("div#jive-body > div#jive-body-intro > h1").length > 0){  //No updates required
				contentId = contentType + ":" + jQuery("div#jive-body > div#jive-body-intro > h1").first().text(); //No updates required
...
function initContentFinding(){
	...
	// PreCloud Migration - jQuery('header#j-header div.socialBarGuest a').attr("data-content-finding", "Community Header");
	// PreCloud Migration - jQuery('header#j-header div.socialBar a').attr("data-content-finding", "Community Header");
	...
	jQuery(document).click(function(evnt) {
		...
		if(jQuerythis.prop("tagName").toLowerCase() == "a"){
			...
			if(jQuerythis.parents("div#fslMainMenu").length > 0){ 
				contentFindingMethod = 'Community Header';
				setContentFinding(jQuerythis, contentFindingMethod);
				return;
			}
